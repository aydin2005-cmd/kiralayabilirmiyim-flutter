...
class _ResultScreenState extends State<ResultScreen> {
  ...
  Future<void> loadResult() async {
    try {
      ...
      if ((statusValue == 'processing' || readyValue == false) &&
          _resultPollAttempts < 30) {
        _resultPollAttempts++;
        await Future.delayed(const Duration(seconds: 2));
        if (!mounted) return;
        return loadResult();
      }
      _resultPollAttempts = 0;
      ...
      shareable = response['shareable'] == true ||
          response['status'] == 'shareable' ||
          response['application_status'] == 'shareable';
      explanationItems = response['explanation_items'] is List
          ? response['explanation_items'] as List<dynamic>
          : [];
      displayMetrics = response['display_metrics'] is Map<String, dynamic>
          ? response['display_metrics'] as Map<String, dynamic>
          : {};
      financialSummary = response['financial_summary'] is Map<String, dynamic>
          ? response['financial_summary'] as Map<String, dynamic>
          : {};
      findeksScoreVisual =
          response['findeks_score_visual'] is Map<String, dynamic>
              ? response['findeks_score_visual'] as Map<String, dynamic>
              : {};
      paymentHabit = response['payment_habit'] is Map<String, dynamic>
          ? response['payment_habit'] as Map<String, dynamic>
          : {};
      reportPersonalization =
          response['report_personalization'] is Map<String, dynamic>
              ? response['report_personalization'] as Map<String, dynamic>
              : {};
      pricingInfo = response['pricing_info'] is Map<String, dynamic>
          ? response['pricing_info'] as Map<String, dynamic>
          : {};
      ...
      if (feeRaw is num) {
        ...
        AppState.instance.serviceFeeAmount =
            num.tryParse(feeRaw.toString().replaceAll(',', '.')) ??
                AppState.instance.serviceFeeAmount;
      }
      AppState.instance.serviceFeeCurrency =
          pricingInfo['service_fee_currency']?.toString() ??
              AppState.instance.serviceFeeCurrency;
  ...
  bool get _isPositive => shareable || resultType == 'positive';
  ...
  String _formatTl(dynamic value) =>
      value == null ? '-' : '${_formatValue(value)} TL';
  String _formatPaymentAmount(dynamic value) =>
      value == null ? '-' : _formatValue(value);

  String _applicationTypeLabel() {
    final raw = AppState.instance.applicationType ??
        displayMetrics['application_type']?.toString() ??
        financialSummary['application_type']?.toString();
    if (raw == 'car_rental') return 'Araç Kiralama';
    return 'Ev Kiralama';
  }

  String _amountTitle() => _applicationTypeLabel() == 'Araç Kiralama'
      ? 'Beyan Edilen Kiralama Tutarı'
      : 'Beyan Edilen Kira Tutarı';

  bool get _isCarRental => _applicationTypeLabel() == 'Araç Kiralama';

  String get _amountSubject => _isCarRental
      ? 'Beyan edilen kiralama tutarı'
      : 'Beyan edilen kira tutarı';

  dynamic _applicationAmount() =>
      financialSummary['application_amount'] ??
      displayMetrics['application_amount'] ??
      AppState.instance.applicationAmount;
  dynamic _capacityIndicator() =>
      financialSummary['average_limit_per_institution'] ??
  ...
  String _debtRatioText() {
    final raw = displayMetrics['debt_limit_ratio_percent'] ??
        financialSummary['debt_limit_ratio_percent'] ??
        displayMetrics['debt_limit_ratio'];
    if (raw == null) return '';
    final n = raw is num
        ? raw.toDouble()
  ...
  String _maskPart(String? value, int minStars) {
    ...
    final prefix =
        norm.length >= 2 ? norm.substring(0, 2) : norm.substring(0, 1);
    final starCount = (norm.length - prefix.length) > minStars
  ...
  Widget _applicantIdentityCard() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          boxShadow: const [
            BoxShadow(
                color: Color(0x07000000), blurRadius: 10, offset: Offset(0, 4))
          ]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Başvuru Sahibi Bilgileri',
            style: TextStyle(
                fontSize: 16, color: _navy, fontWeight: FontWeight.w900)),
        ...
        const Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(
              child: Text(
                  'Başvuru bilgileri, Findeks raporundaki maskeli alanlarla uyumlu bulunmuştur.',
                  style: TextStyle(
                      fontSize: 12,
                      height: 1.35,
      ...
      child: Stack(
        children: [
          Positioned(
            ...
            bottom: -18,
            child:
                Icon(artIcon, size: 120, color: Colors.white.withOpacity(0.10)),
          ),
          Column(
            ...
            children: [
                  ...
                  Expanded(
                    child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 10),
                          const Text(
                            'Findeks finansal göstergeleriniz, beyan edilen kiralama tutarı için olumlu görünmektedir.',
                            style: TextStyle(
                                color: Colors.white,
                                fontSize: 15.5,
                                height: 1.35,
                          ...
                          Container(
                            ...
                            child: const Text(
                                '✓ Sonuç: Paylaşılabilir olumlu sonuç',
                                style: TextStyle(
              ...
              if (locked) ...[
                ...
                Container(
                  ...
                  child: const Text(
                    'Paylaşılabilir olumlu sonuç raporunuz hazır. Tam rapor, PDF ve paylaşım linkleri ödeme sonrasında açılacaktır.',
                    style: TextStyle(
                        color: Colors.white,
                        height: 1.35,
                        fontWeight: FontWeight.w700),
  ...
  Widget _quickSummaryCards() {
    ...
    final items = [
      _SummaryItem(
          ...
          'Kiralama Türü',
          _applicationTypeLabel()),
      _SummaryItem(Icons.payments_outlined, _amountTitle(),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          decoration: const BoxDecoration(
              color: Color(0xFFE9F8F3), shape: BoxShape.circle),
  ...
  void _startPaymentAfterRefundPolicyCheck() {
    if (!refundPolicyAccepted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Lütfen İptal / İade Politikası’nı okuyup onaylayın.'),
        ),
      );
      return;
    }

    _startPaymentFromResult();
  }
              ...
              children: const [
                SizedBox(
                    ...
                    child: CircularProgressIndicator(
                        strokeWidth: 3, color: _teal)),
                SizedBox(height: 18),
                Text('Ödeme durumunuz kontrol ediliyor...',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                        fontSize: 17,
                        fontWeight: FontWeight.w900,
                        color: _navy)),
                SizedBox(height: 8),
                Text('Lütfen kontrol tamamlanana kadar bu ekrandan ayrılmayın.',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                        fontSize: 13,
        ...
        children: reasons
            .map((r) => Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      border: Border.all(color: const Color(0xFFCFEDE5)),
                      borderRadius: BorderRadius.circular(16)),
                  child: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Container(
                          ...
                          height: 32,
                          decoration: const BoxDecoration(
                              color: Color(0xFFDFF7EC), shape: BoxShape.circle),
  ...
  String _paymentOrderLabel() {
    ...
    return paymentHabit['label']?.toString().trim().isNotEmpty == true
        ? paymentHabit['label'].toString()
        : 'Olumlu';
  }
  ...
  Widget _findeksIndicators() {
    ...
    final debtValue =
        debtText.isEmpty ? _debtLoadLabel() : '${_debtLoadLabel()} ($debtText)';
    final capacityValue = _capacityIndicator() == null
        ? _capacityLabel()
        : '${_capacityLabel()} / ${_formatTl(_capacityIndicator())}';

    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _sectionTitle('Findeks Finansal Göstergeleri'),
      Row(children: [
        Expanded(
            child: _metricCard(Icons.speed_rounded, 'Findeks Notu',
      ...
      Row(children: [
        Expanded(
            child: _metricCard(Icons.pie_chart_outline_rounded,
                'Mevcut Borç Yükü', debtValue)),
        const SizedBox(width: 10),
        Expanded(
            child: _metricCard(
                Icons.trending_up_rounded, 'Finansal Kapasite', capacityValue)),
      ]),
      if (score != null) ...[
        const SizedBox(height: 14),
        _findeksScale(score),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _categoryPanel(
        ...
        options: const [
          ...
          _CategoryOption('Yüksek Ama Kabul Edilebilir Borçluluk',
              warning: true),
        ],
      ),
      const SizedBox(height: 12),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...options.map((option) => Padding(
              ...
              child: _categoryOption(option.label,
                  _labelMatches(option.label, selected), option.warning),
            )),
      ]),
    );
  }
  ...
  bool _labelMatches(String a, String b) =>
      _normalizeCategoryLabel(a) == _normalizeCategoryLabel(b);

  String _normalizeCategoryLabel(String value) => value
      .toLowerCase()
      .replaceAll('ı', 'i')
      .replaceAll('ğ', 'g')
      .replaceAll('ü', 'u')
  ...
  Widget _categoryOption(String label, bool selected, bool warning) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: selected
            ? (warning ? const Color(0xFFFFF3E0) : const Color(0xFFE9F8F3))
            : const Color(0xFFF8FAFC),
        border: Border.all(
            color: selected
                ? (warning ? const Color(0xFFF59E0B) : const Color(0xFF20A978))
      ...
      child: Row(children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: selected
                ? (warning ? const Color(0xFFF59E0B) : _green)
                : Colors.white,
            shape: BoxShape.circle,
            border: Border.all(
                color: selected
                    ? (warning ? const Color(0xFFF59E0B) : _green)
                    : const Color(0xFFC8D5E3)),
            boxShadow: selected
                ? [
                    BoxShadow(
                        color: warning
                            ? const Color(0x33F59E0B)
                            : const Color(0x33108A56),
                        blurRadius: 12,
    ...
    return _sectionCard(
      ...
      child: LayoutBuilder(builder: (context, constraints) {
        ...
        return Column(children: [
            ...
            Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: const [
                    BoxShadow(color: Color(0x22008A55), blurRadius: 6)
                  ],
                ),
              ),
            ),
          ...
          const SizedBox(height: 8),
          const Text(
              'Findeks notu, bu değerlendirmede olumlu aralıkta görünmektedir.',
              textAlign: TextAlign.center,
  ...
  Widget _rentEvaluation() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _sectionTitle(_isCarRental
          ? 'Kiralama Tutarı Değerlendirmesi'
      ...
      _sectionCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(
                child: _inlineData(
                    'Finansal kapasite göstergesi', _formatTl(capacity))),
          ]),
          const SizedBox(height: 12),
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(
                child: Text(
                    '${_rentAlignmentLabel()}: ${_profileComment('rent_alignment', '$_amountSubject, raporda görülen finansal kapasite göstergeleriyle uyumlu görünmektedir.')}',
                    style: const TextStyle(
                        height: 1.35,
                        fontWeight: FontWeight.w700,
                        color: Color(0xFF075E47)))),
  ...
  Widget _financialSummary() {
    final institutionCount = financialSummary['institution_count'] ??
        displayMetrics['institution_count'];
    final totalLimits =
        financialSummary['total_limits'] ?? displayMetrics['total_limits'];
    final totalDebts =
        financialSummary['total_debts'] ?? displayMetrics['total_debts'];
    final capacity = _capacityIndicator();

    if (institutionCount == null &&
        totalLimits == null &&
        totalDebts == null &&
        capacity == null) return const SizedBox.shrink();
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GridView.count(
        ...
        childAspectRatio: 1.05,
        children: [
          _summaryTile(_SummaryItem(Icons.account_balance_outlined,
              'Kredi Veren Kurum Sayısı', _formatValue(institutionCount))),
          _summaryTile(_SummaryItem(Icons.account_balance_wallet_outlined,
              'Toplam Finansal Limit', _formatTl(totalLimits))),
          _summaryTile(_SummaryItem(Icons.credit_card_outlined,
              'Toplam Mevcut Borç', _formatTl(totalDebts))),
          _summaryTile(_SummaryItem(Icons.trending_up_rounded,
              'Finansal Kapasite Göstergesi', _formatTl(capacity))),
        ],
      ),
    ]);
      ...
      _sectionCard(
        child: const Column(children: [
          _VerificationRow(
              ...
              text: 'Rapor sahibi doğrulandı'),
          _VerificationRow(
              icon: Icons.event_available_outlined, text: 'Rapor güncel'),
          _VerificationRow(
              icon: Icons.person_search_outlined,
              text: 'Başvuru bilgileriyle uyumlu'),
          _VerificationRow(
              icon: Icons.picture_as_pdf_outlined,
              text: 'PDF format kontrolü yapıldı'),
        ]),
      ),
    ]);
  }
  ...
  Widget _disclaimer() {
    return Container(
      ...
      child:
        ...
        SizedBox(height: 6),
        Text(
          'Bu rapor, başvuru sahibinin kendi rızasıyla yüklediği Findeks risk raporu ve beyan ettiği kiralama tutarı esas alınarak oluşturulmuş otomatik bir finansal değerlendirme raporudur. Garanti, kefalet veya ödeme taahhüdü içermez. Kiralama kararının nihai takdiri kiraya veren kişi veya kuruma aittir.',
  ...
  Widget _negativeResult() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [
          BoxShadow(
              color: Color(0x12000000), blurRadius: 22, offset: Offset(0, 10))
        ],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Center(
            child: Container(
                ...
                child: const Icon(Icons.fact_check_outlined,
                    size: 40, color: Color(0xFFD46A1F)))),
        const SizedBox(height: 16),
        Center(
            child: Text(_negativeTitle,
                textAlign: TextAlign.center,
                style: const TextStyle(
                    fontSize: 24, fontWeight: FontWeight.w900, color: _navy))),
        const SizedBox(height: 6),
        const Center(
            child: Text('Bu başvuru için olumlu sonuç oluşmadı',
                textAlign: TextAlign.center,
                style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.w900,
        ...
        Container(
            ...
            decoration: BoxDecoration(
                ...
                border: Border.all(color: _border)),
            child: const Text(
                'Bu sonuç teknik bir hata değildir. Değerlendirme, yüklenen rapordaki veriler ve beyan edilen tutar üzerinden otomatik olarak yapılmıştır.',
                style: TextStyle(
                    fontSize: 13,
                    height: 1.35,
                    color: Color(0xFF334155),
